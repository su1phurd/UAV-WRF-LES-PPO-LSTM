...
from config import (
    ...
    GAUSSIAN_RADIUS, PEAK_CONCENTRATION
)
...
class MethaneEnv(gym.Env):
    ...
    def _generate_plume(self):
        ...
        # 高斯分布主流程，便于NetCDF和LSTM
        base = PEAK_CONCENTRATION * np.exp(-dist**2/(2*(GAUSSIAN_RADIUS)**2))
        ...
        self.conc_field = np.clip(base + turbulence, 0, PEAK_CONCENTRATION)
        self.tke_field = turbulence
        self.gaussian_params = {
            'mu_x': self.source_pos[0],
            'mu_y': self.source_pos[1],
            'sigma': GAUSSIAN_RADIUS,
            'peak': PEAK_CONCENTRATION
        }
    ...
    def step(self, action):
        ...
        prev_conc = self.conc_field[prev_x, prev_y] / PEAK_CONCENTRATION
        ...
        current_conc = self.conc_field[current_x, current_y] / PEAK_CONCENTRATION
